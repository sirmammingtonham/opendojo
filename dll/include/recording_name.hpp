...
inline std::size_t prefix_size(std::string_view name) {
    ...
    for (std::size_t count = 0; count < MAX_CHARS && pos < name.size(); ++count) {
        ...
        if (lead < 0x80) { length = 1; cp = lead; minimum = 0; }
        else if (lead >= 0xC2 && lead <= 0xDF) { length = 2; cp = lead & 0x1F; minimum = 0x80; }
        else if (lead >= 0xE0 && lead <= 0xEF) { length = 3; cp = lead & 0x0F; minimum = 0x800; }
        else if (lead >= 0xF0 && lead <= 0xF4) { length = 4; cp = lead & 0x07; minimum = 0x10000; }
        else break;
        ...
        if (cp < 0x20 || (cp >= 0x7F && cp <= 0x9F) ||
            ...
            (cp >= 0x2060 && cp <= 0x206F) || cp == 0xFEFF) break;
        pos += length;
    }
    return pos;
}
...
inline std::string from_file(std::string_view name) {
    ...
    if (normalized.size() == 6 && normalized.compare(0, 5, "slot ") == 0 &&
        normalized[5] >= '1' && normalized[5] <= '8') return {};
    return normalized;
}
